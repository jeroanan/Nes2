...
class TestCpyOpCode(OpCodeTestBase):
    ...
    def test_cpy_immediate_command_sets_zero_flag_when_comparison_is_equal(self):
        self.target.execute(OpCodeDefinitions.cpy_immediate_command, 0x0)
    ...
    def test_cpy_immediate_command_does_not_set_zero_flag_when_comparison_is_unequal(self):
        self.target.execute(OpCodeDefinitions.cpy_immediate_command, 0x1)
    ...
    def test_cpy_zero_page_command_calls_cpy_method(self):
        self.assert_opcode_execution(OpCodeDefinitions.cpy_zero_page_command, self.target.get_cpy_command_executed)

    def test_cpy_absolute_command_calls_cpy_method(self):
        self.assert_opcode_execution(OpCodeDefinitions.cpy_absolute_command, self.target.get_cpy_command_executed)
